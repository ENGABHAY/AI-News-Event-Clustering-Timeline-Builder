#!/usr/bin/env python3
"""
News Event Clustering & Summarization Pipeline
================================================

Converts the "News_clustering" notebook into a runnable, end-to-end script.

Given a JSON (JSON-Lines) or CSV file of news articles, this pipeline will:

  1. Load & clean the data
  2. Run exploratory data analysis (plots saved to disk)
  3. Build a combined text field
  4. Generate sentence embeddings (Sentence-Transformers)
  5. Reduce dimensionality (UMAP) and cluster (HDBSCAN) into "events"
  6. Label each cluster with TF-IDF keywords and a representative headline
  7. Build an event-level summary table
  8. Visualize event statistics (plots saved to disk)
  9. (Optional) Generate natural-language event summaries using an LLM
     (Google Gemini) and/or a pretrained BART summarizer

All logic mirrors the original notebook — no steps have been added, removed,
or reinterpreted beyond what is needed to make it run as a standalone script
on an arbitrary input file.

--------------------------------------------------------------------------
REQUIRED INPUT COLUMNS
--------------------------------------------------------------------------
The input file (JSON-Lines or CSV) must contain the following columns,
matching the structure used in the source notebook (News Category Dataset):

    - headline            (str)
    - short_description   (str)
    - authors              (str)
    - date                 (str / datetime-parsable)
    - category             (str)
    - link                 (str, optional)
...
USAGE
--------------------------------------------------------------------------
Basic run (cleaning + clustering + event table only):

    python news_event_clustering_pipeline.py --input news.json --output-dir results

Full run including EDA plots and LLM/BART summarization of the top events:

    python news_event_clustering_pipeline.py \\
        --input news.csv \\
        --output-dir results \\
        --run-eda \\
        --summarize llm \\
...
--------------------------------------------------------------------------
    cleaned_data.csv          Cleaned input data
    news_embeddings.npy       Sentence embeddings (cached, reusable)
    news_labels.npy           Cluster labels (cached, reusable)
    news_clustered.csv        Article-level data with cluster/keywords/event_label
    event_table.csv           One row per event (cluster)
    event_table_summarized.csv  Event table + generated summaries (if --summarize)
    plots/                    EDA and event-analysis plots (if --run-eda)
"""
...
def run_pipeline(args):
    ...
    df = clean_data(df)
    df.to_csv(os.path.join(args.output_dir, "cleaned_data.csv"), index=False)
    ...
    df.to_csv(os.path.join(args.output_dir, "news_clustered.csv"), index=False)

    # 8. Event table
    event_table = build_event_table(df)
    event_table.to_csv(os.path.join(args.output_dir, "event_table.csv"), index=False)

    if args.run_eda:
        visualize_events(event_table, plots_dir)

    # 9. Summarization (optional)
    if args.summarize == "llm":
        if not args.gemini_api_key:
            print("ERROR: --gemini-api-key is required for --summarize llm", file=sys.stderr)
            sys.exit(1)
        summarized = summarize_with_llm(
            df, embeddings, event_table, args.gemini_api_key, top_n=args.top_n_events
        )
        summarized.to_csv(
            os.path.join(args.output_dir, "event_table_summarized.csv"), index=False
        )
        print(f"Summarized top {args.top_n_events} events using Gemini LLM")

    elif args.summarize == "bart":
        summarized = summarize_with_bart(df, event_table, top_n=args.top_n_events)
        summarized.to_csv(
            os.path.join(args.output_dir, "event_table_summarized.csv"), index=False
        )
        print(f"Summarized top {args.top_n_events} events using BART")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="News Event Clustering & Summarization Pipeline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--input", "-i", required=True,
        help="Path to input news data file (.json / .jsonl / .csv)",
    )
    parser.add_argument(
        "--output-dir", "-o", default="pipeline_output",
        help="Directory to write all outputs to (default: pipeline_output)",
    )
    parser.add_argument(
        "--run-eda", action="store_true",
        help="Generate and save EDA / event analysis plots",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Ignore cached embeddings/labels and recompute from scratch",
    )
    parser.add_argument(
        "--summarize", choices=["none", "llm", "bart"], default="none",
        help="Generate natural-language event summaries for the top events "
             "using Gemini ('llm') or a pretrained BART model ('bart')",
    )
    parser.add_argument(
        "--gemini-api-key", default=None,
        help="Google Gemini API key (required if --summarize llm)",
    )
    parser.add_argument(
        "--top-n-events", type=int, default=20,
        help="Number of largest events to summarize (default: 20)",
    )
    return parser
